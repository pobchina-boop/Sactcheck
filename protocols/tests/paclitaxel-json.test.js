...
assert.ok(missingDuration.missing.some(item => item.id === 'anc_below_0_5_duration_days'));
console.log('✓ ANC duration becomes conditionally mandatory only when ANC is below 0.5');
...
assert.ok(missingOtherCurrent.missing.some(item => item.id === 'other_nonhaematological_current_grade'));
assert.ok(missingOtherCurrent.missing.some(item => item.id === 'previous_dose_reductions'));
console.log('✓ grade 2 toxicity reveals and requires its dependent fields');
...
assert.equal(optionalGaps.complete, true);
...
console.log(`\n${cases.length + 5} weekly paclitaxel JSON tests passed.`);
