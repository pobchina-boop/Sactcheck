...
console.log('✓ optional olaparib domains no longer block the core assessment');
...
assert.ok(result.missing.some(item => item.id === 'febrile_neutropenia_grade'));
assert.ok(result.missing.some(item => item.id === 'haematological_toxicity_occurrence_number'));
console.log('✓ febrile neutropenia activates only its relevant dependent fields');
...
assert.equal(result.complete, false);
assert.ok(result.missing.some(item => item.id === 'haemodialysis'));
console.log('✓ haemodialysis becomes mandatory only in severe renal impairment');
