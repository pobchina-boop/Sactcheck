...
assert.strictEqual(index.protocol_count, 210, 'Complete protocol index must contain 210 distinct protocols.');
assert.strictEqual(index.protocols.length, 210, 'Complete protocol index array must contain 210 entries.');

const riskMap = JSON.parse(fs.readFileSync(path.join(root, 'data', 'emetogenic-risk-map.json'), 'utf8'));
assert.strictEqual(riskMap.release, '0.39.0');
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 210, 'Central supportive-care map must cover all 210 protocols.');
...
for (const { file, data } of gi) {
  ...
  assert.strictEqual(metadata.sactcheck_encoding_version, '0.39.0', `${code} lacks the v0.39.0 GI encoding marker.`);
...
assert.strictEqual(Aliases.version, '0.39.0');
...
assert(html.includes('Version 0.39.0 · complete gastrointestinal library'), 'v0.39.0 release badge is missing.');
assert(html.includes('js/protocol-loader.js?v=0.39.0'), 'v0.39.0 cache key is missing.');
assert(html.includes('js/drug-aliases.js?v=0.39.0'), 'v0.39.0 alias cache key is missing.');
