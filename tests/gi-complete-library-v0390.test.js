...
assert.strictEqual(index.protocol_count, 338, 'Complete protocol index must contain 210 distinct protocols.');
assert.strictEqual(index.protocols.length, 338, 'Complete protocol index array must contain 210 entries.');
...
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 338, 'Central supportive-care map must cover all 210 protocols.');
...
assert(html.includes('Version 0.45.2 · one-page clinical output and safety-language update'), 'v0.39.0 release badge is missing.');
