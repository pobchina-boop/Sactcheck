...
assert.strictEqual(index.protocol_count, 329, 'Complete protocol index must contain 210 distinct protocols.');
assert.strictEqual(index.protocols.length, 329, 'Complete protocol index array must contain 210 entries.');
...
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 329, 'Central supportive-care map must cover all 210 protocols.');
...
for (const { file, data } of gi) {
  ...
  assert(['0.39.0','0.40.0','0.41.0','0.43.0','0.44.0'].includes(metadata.sactcheck_encoding_version), `${code} lacks a supported GI/Lung encoding marker.`);
...
assert(html.includes('Version 0.44.0 · complete Genitourinary library'), 'v0.39.0 release badge is missing.');
assert(html.includes('js/protocol-loader.js?v=0.44.0'), 'v0.39.0 cache key is missing.');
assert(html.includes('js/drug-aliases.js?v=0.44.0'), 'v0.39.0 alias cache key is missing.');
