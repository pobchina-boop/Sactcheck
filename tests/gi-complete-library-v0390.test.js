...
assert.strictEqual(index.protocol_count, 359, 'Complete protocol index must contain 210 distinct protocols.');
assert.strictEqual(index.protocols.length, 359, 'Complete protocol index array must contain 210 entries.');
...
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 359, 'Central supportive-care map must cover all 210 protocols.');
...
for (const { file, data } of gi) {
  ...
  assert(['0.39.0','0.40.0','0.41.0','0.43.0','0.44.0','0.45.0', '0.45.1','0.46.0'].includes(metadata.sactcheck_encoding_version), `${code} lacks a supported GI/Lung encoding marker.`);
...
assert.strictEqual(Aliases.version, '0.46.0');
...
assert(html.includes('Version 0.46.0 · complete Head and Neck library'), 'v0.39.0 release badge is missing.');
assert(html.includes('js/protocol-loader.js?v=0.46.0'), 'v0.39.0 cache key is missing.');
assert(html.includes('js/drug-aliases.js?v=0.46.0'), 'v0.39.0 alias cache key is missing.');
