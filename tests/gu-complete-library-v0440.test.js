...
const cardSidecar=read('data/regimen-card-metadata.json');assert.strictEqual(cardSidecar.protocol_count,329);
...
for(const {entry,data} of gu){
 ...
 if(newlyAdded.has(code)){newCount++;assert(entry.path.startsWith('protocols/genitourinary/'),`${code} new file path`);assert(m.sactcheck_encoding_version==='0.44.0',`${code} encoding version`);}
}
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('Version 0.44.0 · complete Genitourinary library'));assert(html.includes('js/protocol-loader.js?v=0.44.0'));
