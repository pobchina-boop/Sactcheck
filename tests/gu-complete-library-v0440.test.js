...
for(const {entry,data} of gu){
 ...
 if(newlyAdded.has(code)){newCount++;assert(entry.path.startsWith('protocols/genitourinary/'),`${code} new file path`);assert(['0.44.0','0.45.0','0.45.1','0.46.0','0.47.0'].includes(m.sactcheck_encoding_version),`${code} encoding version`);}
}
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('v0.52.4 · What changed?'));assert(html.includes('js/protocol-loader.js?v=0.51.0'));
