...
const risk=read('data/emetogenic-risk-map.json');assert.strictEqual(risk.release,'0.47.2');assert.strictEqual(Object.keys(risk.protocols||{}).length,361);for(const c of ['00320','00327','00642','00702','00758'])assert(risk.protocols[c]);
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('v0.47.2 · What changed?'));assert(html.includes('js/tissue-ui.js?v=0.47.2'));assert(html.includes('js/protocol-context.js?v=0.47.2'));
