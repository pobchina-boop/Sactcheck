...
const risk=read('data/emetogenic-risk-map.json');assert.strictEqual(risk.release,'0.47.0');assert.strictEqual(Object.keys(risk.protocols||{}).length,361);for(const c of ['00320','00327','00642','00702','00758'])assert(risk.protocols[c]);
const sidecar=read('data/regimen-card-metadata.json');assert.strictEqual(sidecar.protocol_count,361);for(const d of [...net,...ta].map(x=>x.data))assert(sidecar.protocols.some(x=>x.id===d.protocol_id));
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('Version 0.47.0 · complete Neuroendocrine and adult tumour-agnostic libraries'));assert(html.includes('js/tissue-ui.js?v=0.47.0'));assert(html.includes('js/protocol-context.js?v=0.47.0'));
