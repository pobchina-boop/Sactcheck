...
const risk=read('data/emetogenic-risk-map.json');assert.strictEqual(risk.release,'0.48.0');assert.strictEqual(Object.keys(risk.protocols||{}).length,366);for(const c of ['00320','00327','00642','00702','00758'])assert(risk.protocols[c]);
const sidecar=read('data/regimen-card-metadata.json');assert.strictEqual(sidecar.protocol_count,366);for(const d of [...net,...ta].map(x=>x.data))assert(sidecar.protocols.some(x=>x.id===d.protocol_id));
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('v0.49.0 · What changed?'));assert(html.includes('js/tissue-ui.js?v=0.48.4'));assert(html.includes('js/protocol-context.js?v=0.48.4'));
