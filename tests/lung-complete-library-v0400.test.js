...
for (const { file, data } of lung) {
  ...
  assert(['0.40.0','0.43.0','0.44.0','0.45.0','0.45.1','0.46.0'].includes(m.sactcheck_encoding_version),`${code} lacks a supported Lung/Gynaecology reconciliation encoding marker.`);
...
assert(html.includes('Version 0.46.0 · complete Head and Neck library'),'current release badge missing.');
assert(html.includes('js/protocol-loader.js?v=0.46.0'),'current loader cache key missing.');
assert(html.includes('js/drug-aliases.js?v=0.46.0'),'current alias cache key missing.');
