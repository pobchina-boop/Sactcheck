...
for(const f of files){const p=path.join(root,'protocols','breast',f);const d=JSON.parse(fs.readFileSync(p,'utf8'));assert.equal(d.metadata.sactcheck_encoding_version,'0.37.0');assert(d.metadata.treatment_class?.length);assert(d.ctcae_standard);assert(d.supportive_care?.supportive_medications_pdf_url);}
