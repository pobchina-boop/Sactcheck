...
assert(html.includes('<title>SACTCheck v0.52.3 — Input Classification &amp; ECOG Guidance</title>'));
assert(html.includes('<span class="header-version">v0.52.3</span>'));
assert(html.includes('css/protocol-dose-schedule.css?v=0.52.3'));
assert(html.includes('js/protocol-dose-schedule.js?v=0.52.3'));
assert(html.indexOf('js/protocol-dose-schedule.js?v=0.52.3') < html.indexOf('js/generic-assessment-ui.js?v=0.52.3'));
...
assert.strictEqual(context.SACTCheckProtocolDoseSchedule.version, '0.52.3');
assert(!/<input[^>]+type=["']number/i.test(doseSource));
assert(!/oncoassist/i.test(doseSource));

console.log('v0.52.0 Dose & Schedule integration baseline retained under the v0.52.3 integrated engine.');
