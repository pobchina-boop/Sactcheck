...
const integrity = JSON.parse(fs.readFileSync(path.join(root, 'V0580_PROTOCOL_JSON_HASHES.json'), 'utf8'));
...
assert.strictEqual(integrity.unchanged_from_baseline, true, 'Protocol JSON files must be unchanged from v0.57.0.');
assert.deepStrictEqual(protocolHashes, integrity.hashes, 'Current protocol JSON hashes do not match the v0.58.0 integrity register.');
