...
for (const { file, data } of protocols) {
  ...
  assert(['0.37.0', '0.38.0', '0.38.2', '0.39.0', '0.40.0', '0.41.0', '0.41.1', '0.42.0', '0.43.0', '0.44.0', '0.45.0', '0.45.1', '0.46.0','0.47.0','0.49.0','0.50.0'].includes(metadata.sactcheck_encoding_version), `${code} has unsupported SACTCheck encoding version ${metadata.sactcheck_encoding_version}`);
  ...
  for (const [field, def] of Object.entries(definitions)) {
    ...
    if (mode === 'exact_continuous') {
      ...
      assert(/carboplatin/i.test(`${metadata.title || ''} ${(metadata.drugs || []).join(' ')}`), `${code}/${field} exact renal exception is not carboplatin-related`);
