...
assert.equal(protocols.length, 270, `Expected 270 regimen protocols, found ${protocols.length}`);
...
for (const { file, data } of protocols) {
  ...
  assert(['0.37.0', '0.38.0', '0.38.2', '0.39.0', '0.40.0', '0.41.0'].includes(metadata.sactcheck_encoding_version), `${code} has unsupported SACTCheck encoding version ${metadata.sactcheck_encoding_version}`);
...
assert.equal(Object.keys(riskMap.protocols || {}).length, 270, 'Central supportive-care map must cover every regimen.');
...
assert(html.includes('Version 0.41.0 · complete sarcoma library'), 'Release badge is stale.');
assert(html.includes('js/protocol-loader.js?v=0.41.0'), 'Protocol loader cache key is stale.');
