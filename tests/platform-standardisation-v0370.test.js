...
assert.equal(protocols.length, 248, `Expected 210 regimen protocols, found ${protocols.length}`);
...
for (const { file, data } of protocols) {
  ...
  assert(['0.37.0', '0.38.0', '0.38.2', '0.39.0', '0.40.0'].includes(metadata.sactcheck_encoding_version), `${code} has unsupported SACTCheck encoding version ${metadata.sactcheck_encoding_version}`);
...
assert.equal(Object.keys(riskMap.protocols || {}).length, 248, 'Central supportive-care map must cover every regimen.');
...
assert(html.includes('Version 0.40.0 · complete lung library'), 'Release badge is stale.');
assert(html.includes('js/protocol-loader.js?v=0.40.0'), 'Protocol loader cache key is stale.');
