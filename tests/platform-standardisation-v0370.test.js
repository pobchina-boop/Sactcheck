...
assert.equal(protocols.length, 366, `Expected 366 canonical regimen protocols, found ${protocols.length}`);
...
for (const { file, data } of protocols) {
  ...
  assert(['0.37.0', '0.38.0', '0.38.2', '0.39.0', '0.40.0', '0.41.0', '0.41.1', '0.42.0', '0.43.0', '0.44.0', '0.45.0', '0.45.1', '0.46.0','0.47.0','0.49.0'].includes(metadata.sactcheck_encoding_version), `${code} has unsupported SACTCheck encoding version ${metadata.sactcheck_encoding_version}`);
...
assert.equal(variableRisk, 6, 'One solid-tumour regimen plus five haem foundation protocols should remain variable pending component/local pharmacy reconciliation.');

assert.equal(Object.keys(riskMap.protocols || {}).length, 366, 'Central supportive-care map must cover every regimen.');
...
assert(html.includes('v0.49.0 · What changed?'), 'Release badge is stale.');
assert(html.includes('js/protocol-loader.js?v=0.49.0'), 'Protocol loader cache key is stale.');
