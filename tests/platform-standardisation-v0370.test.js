...
assert.equal(protocols.length, 338, `Expected 338 canonical regimen protocols, found ${protocols.length}`);
...
for (const { file, data } of protocols) {
  ...
  assert(['0.37.0', '0.38.0', '0.38.2', '0.39.0', '0.40.0', '0.41.0', '0.41.1', '0.42.0', '0.43.0', '0.44.0', '0.45.0', '0.45.1'].includes(metadata.sactcheck_encoding_version), `${code} has unsupported SACTCheck encoding version ${metadata.sactcheck_encoding_version}`);
...
assert.equal(Object.keys(riskMap.protocols || {}).length, 338, 'Central supportive-care map must cover every regimen.');
...
assert(html.includes('Version 0.45.2 · one-page clinical output and safety-language update'), 'Release badge is stale.');
assert(html.includes('js/protocol-loader.js?v=0.45.1'), 'Protocol loader cache key is stale.');
