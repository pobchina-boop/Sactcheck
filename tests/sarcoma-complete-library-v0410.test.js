...
assert.strictEqual(index.protocol_count, 329,'Complete protocol index must contain 270 protocols.');
assert.strictEqual(index.protocols.length, 329,'Complete protocol index array must contain 270 entries.');
...
assert.strictEqual(Object.keys(riskMap.protocols||{}).length,329,'Supportive-care map must cover all protocols.');
...
for(const {data} of sarcoma){
 ...
 assert(['0.41.0','0.43.0','0.44.0'].includes(m.sactcheck_encoding_version),`${code} lacks a supported Sarcoma/Gynaecology reconciliation encoding marker.`);
...
const Aliases=aliasContext.SACTCheckDrugAliases;assert.strictEqual(Aliases.version,'0.44.0');
...
assert(html.includes('Version 0.44.0 · complete Genitourinary library'),'v0.41.1 release badge missing.');
assert(html.includes('js/protocol-loader.js?v=0.44.0'),'v0.41.1 loader cache key missing.');
assert(html.includes('js/drug-aliases.js?v=0.44.0'),'v0.41.1 alias cache key missing.');
