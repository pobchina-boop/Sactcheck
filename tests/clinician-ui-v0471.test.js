...
assert(index.includes('v0.50.1 · What changed?'), 'Compact release summary is missing');
...
assert(index.includes('js/library-ux.js?v=0.50.1'), 'Library usability module is not loaded');
...
assert(!tissueUi.includes('id: "haem"'), 'Haematology must not appear in Solid Tumour tissue navigation');
assert(tissueUi.includes('card.dataset.libraryDomain !== "haem"'), 'Solid Tumour counts must exclude Haematology cards');
...
assert(assessmentUi.includes('Treatment criteria not met'), 'Concise restrictive outcome wording is missing');
assert(assessmentUi.includes('Treatment criteria met'), 'Concise reassuring outcome wording is missing');
