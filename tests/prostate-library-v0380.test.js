...
assert.strictEqual(index.protocol_count, 210, 'The protocol index must contain 210 distinct regimen protocols.');
assert.strictEqual(index.protocols.length, 210, 'The protocol index array must contain 210 entries.');
assert.strictEqual(new Set(index.protocols.map(item => item.id)).size, 210, 'Protocol index contains duplicate IDs.');
...
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 210, 'Central supportive-care map must cover the entire indexed library.');
...
assert(html.includes('Version 0.39.0 · complete gastrointestinal library'), 'v0.38.0 release badge is missing.');
assert(html.includes('js/drug-aliases.js?v=0.39.0'), 'v0.38.0 alias cache key is missing.');
