...
assert.strictEqual(index.protocol_count, 338, 'The protocol index must contain 210 distinct regimen protocols.');
assert.strictEqual(index.protocols.length, 338, 'The protocol index array must contain 210 entries.');
...
assert.strictEqual(Object.keys(riskMap.protocols || {}).length, 338, 'Central supportive-care map must cover the entire indexed library.');
...
assert(html.includes('Version 0.45.2 · one-page clinical output and safety-language update'), 'v0.38.0 release badge is missing.');
assert(html.includes('js/drug-aliases.js?v=0.45.1'), 'v0.38.0 alias cache key is missing.');
assert.strictEqual(Aliases.version, '0.45.0');
