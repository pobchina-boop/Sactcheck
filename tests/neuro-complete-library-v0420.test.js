...
const aliasCtx={globalThis:null};aliasCtx.globalThis=aliasCtx;vm.createContext(aliasCtx);vm.runInContext(fs.readFileSync(path.join(root,'js/drug-aliases.js'),'utf8'),aliasCtx);const Aliases=aliasCtx.SACTCheckDrugAliases;assert.strictEqual(Aliases.version,'0.47.2');
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('v0.47.2 · What changed?'));assert(html.includes('js/protocol-loader.js?v=0.47.2'));assert(html.includes('js/drug-aliases.js?v=0.47.2'));
