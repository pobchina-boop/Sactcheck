...
const index=read('protocols/index.json');assert.strictEqual(index.protocol_count,280);assert.strictEqual(index.protocols.length,280);assert.strictEqual(new Set(index.protocols.map(x=>x.id)).size,280);
const risk=read('data/emetogenic-risk-map.json');assert.strictEqual(risk.release,'0.42.0');assert.strictEqual(Object.keys(risk.protocols||{}).length,280);
...
const aliasCtx={globalThis:null};aliasCtx.globalThis=aliasCtx;vm.createContext(aliasCtx);vm.runInContext(fs.readFileSync(path.join(root,'js/drug-aliases.js'),'utf8'),aliasCtx);const Aliases=aliasCtx.SACTCheckDrugAliases;assert.strictEqual(Aliases.version,'0.42.0');
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('Version 0.42.0 · complete Neuro-oncology library'));assert(html.includes('js/protocol-loader.js?v=0.42.0'));assert(html.includes('js/drug-aliases.js?v=0.42.0'));
