...
const aliasCtx={globalThis:null};aliasCtx.globalThis=aliasCtx;vm.createContext(aliasCtx);vm.runInContext(fs.readFileSync(path.join(root,'js/drug-aliases.js'),'utf8'),aliasCtx);const Aliases=aliasCtx.SACTCheckDrugAliases;assert.strictEqual(Aliases.version,'0.47.0');
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.includes('Version 0.47.0 · complete Neuroendocrine and adult tumour-agnostic libraries'));assert(html.includes('js/protocol-loader.js?v=0.47.0'));assert(html.includes('js/drug-aliases.js?v=0.47.0'));
