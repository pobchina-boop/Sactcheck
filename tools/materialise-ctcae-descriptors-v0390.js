...
for (const file of walk(path.join(root, 'protocols')).filter(file => file.endsWith('.json'))) {
  ...
  if (!groups(protocol.metadata).includes('Gastrointestinal')) continue;
  let changed=false;
...
console.log(`Materialised toxicity-specific CTCAE descriptions in ${fields} GI fields (${options} options across ${files} protocol files).`);
