/**
 * SACTCheck v0.74.0 - cohesive clinician/patient interface shell.
...
(function(root,factory){
  ...
  function ensureModeSwitcher(){
    ...
    section.innerHTML=`
      ...
      <button type="button" data-sact-mode="patient" aria-pressed="false">
        <span class="mode-icon">◎</span><span><strong>Patient-facing support</strong><small>At-a-glance toxicity · treatment schedule · printable passport</small></span>
      </button>`;
  ...
  function setMode(next){
    ...
    if(sub){
      sub.textContent=mode==="patient"
        ?"Find the exact regimen, then open patient-facing treatment, toxicity and printable passport support."
  ...
  function applyBranding(){
    ...
    if(hero){
      ...
      if(copy&&!copy.querySelector(".v0740-brand-lockup")){
        const lock=root.document.createElement("div");
        lock.className="v0740-brand-lockup";
        lock.innerHTML='<img src="assets/branding/sactcheck-mark.svg" alt=""><div><strong>SACTCheck</strong><span>SACT support at point of care</span></div>';
        copy.prepend(lock);
      }
      ...
      if(lead) lead.textContent="The regimen drives the clinic workflow and the patient-facing treatment passport from one source-linked content model.";
      ...
      const vh=hero.querySelector(".mission-visual-header strong");
      if(vh) vh.textContent="Select. Support. Explain. Verify.";
      const pathway=hero.querySelector(".mission-pathway");
      if(pathway) pathway.innerHTML=`
        <div class="mission-pathway-step"><span class="mission-icon">⌕</span><div><strong>Select</strong><small>Choose the exact NCCP regimen.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon">⌁</span><div><strong>Clinician</strong><small>Assessment, supportive care and administration safety.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon">◎</span><div><strong>Patient</strong><small>Toxicity-at-a-glance, schedule and printable passport.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon">↗</span><div><strong>Verify</strong><small>Keep the source and governance boundary visible.</small></div></div>`;
      const footer=hero.querySelector(".mission-visual-footer");
      if(footer) footer.innerHTML="<span>NCCP-derived</span><span>Regimen-specific</span><span>Patient-agnostic</span><span>Source-linked</span>";
    }
  ...
  function polishWorkflowPanel(){
    ...
    body.querySelectorAll(".workflow-module").forEach(module=>{
      ...
      if(module.classList.contains("patient")){
        h3.textContent="Printable patient passport";
        const p=h3.parentElement?.querySelector("p");
        if(p) p.textContent="Patient-agnostic, regimen-specific passport with schedule, toxicity guide and blank cycle record.";
        ...
        if(!module.querySelector("[data-workflow-print-passport]")){
          ...
          b.textContent="Open patient passport";
