...
(function(root,factory){
  ...
  function loadContent(){ if(!contentPromise) contentPromise=fetchJson(CONTENT_URL).catch(()=>({})); return contentPromise; }
  function loadRisk(){ if(!riskPromise) riskPromise=fetchJson(RISK_URL).catch(()=>({agent_profiles:{},generic_modules:{}})); return riskPromise; }
  ...
  function bodyMapItems(rows){
    const all=(rows||[]).map(r=>`${String(r.id||"")}|${String(r.label||"")}|${String(r.detail||"")}`.toLowerCase()).join(" ");
    const checks=[
      ["Head / hormones","head","headache dizziness confusion hormone endocrine thyroid pituitary adrenal"],
      ["Lungs","chest","cough breathless breathlessness pneumonitis ild chest"],
      ["Skin","skin","skin rash itch blister mouth"],
      ["Gut","abdomen","diarrhoea diarrhea vomiting nausea abdominal colitis bowel"],
      ["Liver","liver","liver hepatitis jaundice dark urine"],
      ["Kidneys / urine","kidney","kidney nephritis urine swelling proteinuria"],
      ["Blood / infection","blood","fever infection neutropenia bruising bleeding"],
      ["Nerves / muscles","legs","weakness neuropathy numbness tingling myositis myasthenia"],
    ];
    return checks.filter(([, , terms])=>terms.split(/\s+/).some(t=>all.includes(t))).map(([label,key])=>({label,key}));
  }

  function bodyMapMarkup(rows){
    const items=bodyMapItems(rows);
    const pills=items.map(item=>`<span class="bodymap-pill ${esc(item.key)}">${esc(item.label)}</span>`).join("") || '<span class="bodymap-pill general">General treatment effects can affect different parts of the body.</span>';
    return `<section class="patient-bodymap-card"><div class="patient-bodymap-copy"><span class="patient-a4-label">Chemo man guide</span><h4>Where side effects may appear</h4><p>Use this simple body map to explain that treatment effects can involve different body systems. The exact important effects for this regimen are shown beside it.</p><div class="bodymap-pill-list">${pills}</div></div><div class="patient-bodymap-figure" aria-hidden="true"><svg viewBox="0 0 180 260" role="img"><circle cx="90" cy="34" r="22"/><rect x="68" y="58" width="44" height="78" rx="18"/><rect x="42" y="70" width="22" height="72" rx="10" transform="rotate(18 42 70)"/><rect x="116" y="70" width="22" height="72" rx="10" transform="rotate(-18 116 70)"/><rect x="70" y="134" width="16" height="92" rx="9" transform="rotate(5 70 134)"/><rect x="94" y="134" width="16" height="92" rx="9" transform="rotate(-5 94 134)"/></svg><div class="bodymap-dot head">Head / hormones</div><div class="bodymap-dot chest">Lungs</div><div class="bodymap-dot skin">Skin</div><div class="bodymap-dot abdomen">Gut</div><div class="bodymap-dot liver">Liver</div><div class="bodymap-dot kidney">Kidneys</div><div class="bodymap-dot blood">Blood</div><div class="bodymap-dot legs">Nerves / muscles</div></div></section>`;
  }
  ...
  function openPrintablePassport(protocol,riskContent){
    ...
    viewer.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>SACTCheck Patient Guide - ${esc(title)}</title><style>@page{size:A4;margin:9mm}*{box-sizing:border-box}body{margin:0;background:#eaf0f2;font-family:Arial,Helvetica,sans-serif;color:#17212b}.toolbar{padding:9px;text-align:center;background:#10354f}.toolbar button{border:0;border-radius:7px;background:#fff;color:#10354f;font-weight:800;padding:9px 14px}.page{width:192mm;min-height:279mm;margin:10px auto;background:#fff;padding:10mm;box-shadow:0 8px 28px rgba(0,0,0,.12)}header{display:flex;justify-content:space-between;gap:12px;border-bottom:4px solid #0b7f7a;padding-bottom:8px}header img{width:38px}h1{font-size:22px;color:#10354f;margin:4px 0}.sub{font-size:9px;color:#60737e}.meds{display:flex;gap:5px;flex-wrap:wrap}.meds span{background:#e7f7f5;color:#0b6f6a;border-radius:99px;padding:4px 7px;font-size:8px;font-weight:700}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:9px 0}.box{border:1px solid #d6e2e7;border-radius:8px;padding:8px}.box h2,.section h2{font-size:12px;color:#10354f;margin:0 0 5px}.phase,.risk,.footer,.box{font-size:8.3px;line-height:1.35}.phase{border-left:3px solid #0b7f7a;padding:5px 7px;background:#f6fafb;margin:4px 0}.phase b,.phase span{display:block}.section{margin-top:8px}.riskgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.risk{border:1px solid #d6e2e7;border-left:4px solid #90a5af;border-radius:7px;padding:6px}.risk.contact{border-left-color:#d49a23}.risk.urgent{border-left-color:#b73743}.risk b,.risk span,.risk small{display:block}.risk b{font-size:8.5px;color:#10354f}.risk small{margin-top:3px;color:#71828a}.general h2{color:#315f78}.agent h2{color:#8b5f0a}.immune h2{color:#6d438d}.urgentbox{margin-top:8px;background:#9c2d37;color:#fff;border-radius:8px;padding:8px;font-size:9px}.qr{display:grid;grid-template-columns:1fr 88px;gap:8px;align-items:center;border-top:1px solid #d6e2e7;margin-top:8px;padding-top:7px}.qr img{width:80px}.qr code{font-size:7px;word-break:break-all}.footer{color:#657780;margin-top:6px}.patient-bodymap-card{display:grid;grid-template-columns:1.15fr .85fr;gap:8px;border:1px solid #d6e2e7;border-radius:8px;padding:8px;background:#fbfdfe;margin:9px 0}.patient-bodymap-card h4{margin:2px 0 4px;color:#10354f;font-size:12px}.patient-bodymap-card p{margin:0 0 5px;font-size:8.2px;line-height:1.4;color:#5b7079}.bodymap-pill-list{display:flex;flex-wrap:wrap;gap:4px}.bodymap-pill{border-radius:999px;background:#eef5f7;padding:3px 6px;font-size:7.3px;font-weight:700;color:#355567}.patient-bodymap-figure{position:relative;min-height:168px;background:linear-gradient(180deg,#eef8f6,#ffffff);border:1px solid #d5e6e3;border-radius:8px;overflow:hidden}.patient-bodymap-figure svg{position:absolute;left:52px;top:12px;width:76px;height:136px;fill:#d4e8e3;stroke:#7aa7a0;stroke-width:2}.bodymap-dot{position:absolute;background:#fff;border:1px solid #d4e1e5;border-radius:999px;padding:2px 5px;font-size:6.9px;color:#4f6873;box-shadow:0 1px 4px rgba(0,0,0,.04)}.bodymap-dot.head{right:8px;top:10px}.bodymap-dot.chest{right:12px;top:45px}.bodymap-dot.skin{right:14px;top:78px}.bodymap-dot.abdomen{right:8px;top:111px}.bodymap-dot.liver{left:8px;top:74px}.bodymap-dot.kidney{left:8px;top:104px}.bodymap-dot.blood{left:10px;bottom:28px}.bodymap-dot.legs{right:12px;bottom:20px}@media print{body{background:#fff}.toolbar{display:none}.page{margin:0;box-shadow:none;width:auto;min-height:0;padding:0}}</style></head><body><div class="toolbar"><button onclick="window.print()">Print / Save PDF</button></div><main class="page"><header><div><div class="sub">SACTCHECK · PATIENT TREATMENT GUIDE</div><h1>${esc(title)}</h1><div class="sub">${code?`NCCP ${esc(code)}${version?` · v${esc(version)}`:""}`:"Regimen-specific information"}</div></div><img src="${esc(logoUrl)}" alt=""></header><div class="grid"><section class="box"><h2>Your medicines</h2><div class="meds">${components.map(x=>`<span>${esc(x)}</span>`).join("")}</div></section><section class="box"><h2>Your treatment schedule</h2>${phaseHtml}</section></div>${bodyMapMarkup(rows)}${general.length?`<section class="section general"><h2>General treatment effects</h2><div class="riskgrid">${cards(general)}</div></section>`:""}${agent.length?`<section class="section agent"><h2>Agent-specific effects</h2><div class="riskgrid">${cards(agent)}</div></section>`:""}${immune.length?`<section class="section immune"><h2>Immunotherapy-related effects</h2><div class="riskgrid">${cards(immune)}</div></section>`:""}<div class="urgentbox"><b>Contact your oncology team urgently</b> for severe or rapidly worsening symptoms, fever or sudden illness, new breathlessness or chest pain, persistent diarrhoea or vomiting, neurological change, significant bleeding, or another symptom your team has told you is urgent.</div><div class="qr"><div><b>More detailed regimen information</b><p>Scan to reopen this regimen guide. The QR contains the regimen identifier only and no patient data.</p><code>${esc(link)}</code></div><a href="${esc(link)}"><img src="${esc(qr)}" alt="Regimen QR"></a></div><div class="footer">Patient information support only. This does not replace your oncology team's advice, formal consent, current NCCP guidance or local emergency instructions.</div></main></body></html>`);
  ...
  async function open(protocol,options={}){
    ...
    try{
      ...
      if(options?.autoPdf || options?.tab==="print"){
        close();
        return openPrintablePassport(live,risk);
      }
  ...
  async function generateConsentPdf(protocol){ return open(protocol,{tab:"print",autoPdf:true}); }
