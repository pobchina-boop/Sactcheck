...
(function(root,factory){
  ...
  const DATA_URL="data/regimen-workflow-v0720.json";
  const PDF_URL="js/supportive-care-pdf-v0720.js?v=0.72.0";
  let dataCache=null;
  ...
  function chooseAntiemeticPlan(protocol,data,riskRecord={}){
    ...
    if(level==="high"){
      if(isAnthracyclineCyclophosphamide(protocol,components)){
        return {status:"available",planId:"high_ac",...data.antiemetic_plans.high_ac};
      }
      if(isCarboplatinAuc4Plus(protocol)||hasTdxd(components)){
        return {status:"available",planId:"high_carboplatin_tdxd",...data.antiemetic_plans.high_carboplatin_tdxd};
      }
      return {status:"available",planId:"high_standard",...data.antiemetic_plans.high_standard};
    }
    if(level==="moderate") return {status:"available",planId:"moderate",...data.antiemetic_plans.moderate};
    if(level==="low") return {status:"available",planId:"low",...data.antiemetic_plans.low};
    if(level==="minimal") return {status:"available",planId:"minimal",...data.antiemetic_plans.minimal};
  ...
  function applyBranding(){
    ...
    if(copy && !copy.querySelector(".workflow-brand-lockup")){
      const brand=root.document.createElement("div");
      brand.className="workflow-brand-lockup";
      brand.innerHTML='<img src="assets/branding/sactcheck-mark.svg" alt=""><div><strong>SACTCheck</strong><span>SACT support at the point of care</span></div>';
      copy.prepend(brand);
    }

    const kicker=hero.querySelector(".study-kicker");
    if(kicker) kicker.textContent="Regimen-driven clinical workflow";
    const h1=hero.querySelector(".mission-hero-copy h1");
    if(h1) h1.textContent="One regimen. One clinical workflow.";
    const lead=hero.querySelector(".mission-hero-lead");
    if(lead) lead.textContent="Select the NCCP regimen and bring assessment, consent, supportive care, administration safety and patient information into one source-linked workspace.";
    const purpose=hero.querySelector(".mission-hero-purpose");
    if(purpose) purpose.innerHTML="<strong>SACT support at the point of care:</strong> the inherent properties of the regimen determine the workflow SACTCheck surfaces.";

    const visualHeader=hero.querySelector(".mission-visual-header strong");
    if(visualHeader) visualHeader.textContent="Assess. Consent. Support. Inform.";
    const pathway=hero.querySelector(".mission-pathway");
    if(pathway){
      pathway.innerHTML=`
        <div class="mission-pathway-step"><span class="mission-icon" aria-hidden="true">✓</span><div><strong>Assess</strong><small>Regimen-specific clinical decision support.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon" aria-hidden="true">✎</span><div><strong>Consent</strong><small>Concise regimen-specific consent output.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon" aria-hidden="true">Rx</span><div><strong>Support</strong><small>Antiemetics and administration safety from regimen properties.</small></div></div>
        <div class="mission-pathway-step"><span class="mission-icon" aria-hidden="true">QR</span><div><strong>Inform</strong><small>Patient-agnostic regimen information and QR architecture.</small></div></div>`;
    }
    const footer=hero.querySelector(".mission-visual-footer");
    if(footer) footer.innerHTML="<span>NCCP-derived</span><span>Regimen-specific</span><span>Clinician-controlled</span><span>Source-linked</span>";

    root.document.title="SACTCheck v0.72.0 - Regimen Workflow Engine";
