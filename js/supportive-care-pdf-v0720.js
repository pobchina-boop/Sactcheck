/**
 * SACTCheck v0.72.0 - Supportive-care prescribing support PDF.
  ...
  function buildPdf(payload){
    ...
    function section(title,items){
      ...
      items.forEach((item,i)=>{
        ...
        p.text(medicineLine(item),M+24,p.y,{size:8.4,bold:true,c:RGB.dark});
        p.y-=15;
      });
      p.y-=5;
    }
    ...
    p.para("Verify the current NCCP regimen, patient-specific factors, allergies, interactions, QT risk, renal/hepatic function and local policy before prescribing. Routine mouthwash, antifungal and antidiarrhoeal items from older local proformas are not automatically carried forward because they are not part of the NCCP V8 standard antiemetic table.",M,31,CW,{size:6.1,leading:7,c:RGB.muted,max:3});
